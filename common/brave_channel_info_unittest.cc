...
TEST(BraveChannelInfoTest, DefaultUserDataDirectoryAndChannelTest) {
  ...
  EXPECT_TRUE(chrome::GetDefaultUserDataDirectory(&path));
  EXPECT_EQ("Kahf-Browser", path.BaseName().AsUTF8Unsafe());
  ...
  EXPECT_TRUE(chrome::GetDefaultUserDataDirectory(&path));
  EXPECT_EQ("Kahf-Browser-Beta", path.BaseName().AsUTF8Unsafe());
  ...
  EXPECT_TRUE(chrome::GetDefaultUserDataDirectory(&path));
  EXPECT_EQ("Kahf-Browser-Dev", path.BaseName().AsUTF8Unsafe());
  ...
  EXPECT_TRUE(chrome::GetDefaultUserDataDirectory(&path));
  EXPECT_EQ("Kahf-Browser-Nightly", path.BaseName().AsUTF8Unsafe());
  EXPECT_EQ(version_info::Channel::CANARY, chrome::GetChannel());
  EXPECT_EQ(BRAVE_LINUX_CHANNEL_NIGHTLY,
            chrome::GetChannelName(chrome::WithExtendedStable(false)));
#else   // OFFICIAL_BUILD
  EXPECT_TRUE(chrome::GetDefaultUserDataDirectory(&path));
  EXPECT_EQ("Kahf-Browser-Development", path.BaseName().AsUTF8Unsafe());
